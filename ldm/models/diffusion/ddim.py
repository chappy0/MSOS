...
from ldm.models.diffusion.step_optim import NoiseScheduleVP, StepOptim
...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True, custom_timesteps=None):
        # 如果提供了自定义时间步，则直接使用
        if custom_timesteps is not None:
            print(f'use the pass timestep:{custom_timesteps}')
    ...
    def sample(self,
        ...
        self.make_schedule(ddim_num_steps=S, ddim_eta=eta, verbose=verbose, custom_timesteps=timesteps)
        # sampling
        C, H, W = shape
        size = (batch_size, C, H, W)
        print(f'Data shape for DDIM sampling is {size}, eta {eta}')
        # print(f"progbar:{kwargs['progbar']}")
        # print(f"before ddim_sampling size.shape:{size}")
        samples, intermediates = self.ddim_sampling(conditioning, size,
                                                    ...
                                                    ucg_schedule=ucg_schedule,noprogbar=noprogbar,**kwargs
                                                    )
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      ucg_schedule=None,noprogbar=False,**kwargs):
        #class_name = self.__class__.__name__
        #function_name = sys._getframe().f_code.co_name
        #print(f"Executing {function_name} in class {class_name}")
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                      ...
                                      dynamic_threshold=dynamic_threshold)
            img, pred_x0 = outs
            # print(f"callback,img_callback:{callback,img_callback}")
            if callback: callback(i)
            if img_callback: img_callback(pred_x0, i)

            if index % log_every_t == 0 or index == total_steps - 1 or 'dis_inter' in kwargs:
